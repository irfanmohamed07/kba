document.addEventListener('DOMContentLoaded', () => {
  ...
  links.forEach(link => {
    link.addEventListener('click', (e) => {
      ...
      // Fetch mess prediction data if mess management is clicked
      if (sectionId === 'messmanagement-section') {
        fetchMessPrediction();
      }
    });
  });

  // Handle mobile menu for mess prediction too
  const mobileMenuItems = document.querySelectorAll(".menu-content a");
  mobileMenuItems.forEach(item => {
    item.addEventListener("click", function () {
      const targetSectionId = item.getAttribute("data-section");
      if (targetSectionId === 'messmanagement-section') {
        fetchMessPrediction();
      }
    });
  });

  async function fetchMessPrediction() {
    try {
      const response = await fetch('/api/mess-prediction');
      if (response.ok) {
        const data = await response.json();
        document.getElementById('mess-total-students').textContent = data.totalStudents;
        document.getElementById('mess-away-students').textContent = data.awayOnGatepass;
        document.getElementById('mess-medical-leaves').textContent = data.onMedicalLeave;
        document.getElementById('mess-predicted-attendance').textContent = data.expectedAttendance;
      }
    } catch (error) {
      console.error('Error fetching mess prediction:', error);
    }
  }
});
